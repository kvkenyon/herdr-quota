...
- Herdr resolves pane commands through `PATH`, not a plugin-root working directory. Keep the Rust action and dashboard manifest commands as `sh -c` wrappers rooted at `$HERDR_PLUGIN_ROOT`; the installed action must be checked in a named lab.
