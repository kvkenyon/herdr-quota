...
- Preferences persist only the finite schema in `src/settings.ts`. Visibility/order/meter mode are presentation inputs: do not let them alter collection, retry, upstream quota semantics, or history storage.
