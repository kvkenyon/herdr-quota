...
- Rust refresh lifecycle and display-only age ticks are isolated in `rust/herdr-quota/src/scheduler.rs`; the collector-facing `RefreshWorker` fence and completion-relative timing are covered by paused-time `cargo test --workspace` tests. Product-owned live state reduces through `rust/herdr-quota/src/app_state.rs`; pane lifetime, input, Preferences, and terminal restoration are owned by `rust/herdr-quota/src/ui/{runtime,keys,preferences,terminal}.rs`.
...
- Essential TUI evidence uses the terminal default foreground; severity and selection must remain explicit through weight, markers, and text. Rust overview/detail and input contracts live in `rust/herdr-quota/src/ui/render.rs`; ANSI/NO_COLOR contrast contracts live in `test/render.test.js` and `test/personalization-render.test.js`.
